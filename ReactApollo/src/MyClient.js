...
class MyClient extends Component {
  constructor (props) {
    ...
    this.client.query({
      ...
      data.data.__type && data.data.__type.fields && data.data.__type.fields.forEach(makeForms)

      const fields = Object.values(Forms).reduce((memo, i) => memo.concat(i), [])
      console.log(fields)

      this.props.handleReceive({
        Forms,
        fields,
        validations: {}
      })
